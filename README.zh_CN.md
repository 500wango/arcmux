...
- 上游返回 `429`、额度不足等可重试错误后，重试会排除本次请求已经失败的渠道，转发到同优先级的其他渠道；同优先级渠道耗尽后才会按优先级切换到下一层。
- 将“失败重试次数”设为至少 `1`，否则请求失败后不会触发备用渠道。默认自动重试状态码已包含 `429`；如上游使用其他额度耗尽状态码，请在 `自动重试状态码` 中补充。
