...
func (*StripeAdaptor) RequestPay(c *gin.Context, req *StripePayRequest) {
	...
	if _, err := common.QuotaFromDecimalStrict(decimal.NewFromFloat(chargedMoney).
		Mul(decimal.NewFromFloat(common.QuotaPerUnit))); err != nil {
		c.JSON(http.StatusOK, gin.H{"message": "充值额度超出范围", "data": 10})
		return
	}
	...
	if err != nil {
		...
		if updateErr := topUp.Update(); updateErr != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("Stripe 标记充值订单失败状态失败 trade_no=%s error=%q", referenceId, updateErr.Error()))
		}
...
func StripeWebhook(c *gin.Context) {
	...
	case stripe.EventTypeCheckoutSessionCompleted:
		fulfillmentErr = sessionCompleted(ctx, event, callerIp)
	case stripe.EventTypeCheckoutSessionExpired:
		sessionExpired(ctx, event)
	case stripe.EventTypeCheckoutSessionAsyncPaymentSucceeded:
		fulfillmentErr = sessionAsyncPaymentSucceeded(ctx, event, callerIp)
	...
	if fulfillmentErr != nil {
		logger.LogError(ctx, fmt.Sprintf("Stripe webhook 履约失败 event_type=%s client_ip=%s error=%q", string(event.Type), callerIp, fulfillmentErr.Error()))
...
func fulfillOrder(ctx context.Context, event stripe.Event, referenceId string, customerId string, callerIp string) error {
	if len(referenceId) == 0 {
		...
		return errors.New("stripe checkout is missing client_reference_id")
	}
...
func getStripeMinTopup() int64 {
	...
	if operation_setting.GetQuotaDisplayType() == operation_setting.QuotaDisplayTypeTokens {
		quotaMin, err := common.QuotaFromDecimalStrict(decimal.NewFromInt(int64(minTopup)).
