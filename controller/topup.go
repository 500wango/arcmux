...
func getMinTopup() int64 {
	...
	if operation_setting.GetQuotaDisplayType() == operation_setting.QuotaDisplayTypeTokens {
		...
		dQuotaPerUnit := decimal.NewFromFloat(common.QuotaPerUnit)
		quotaMin, err := common.QuotaFromDecimalStrict(dMinTopup.Mul(dQuotaPerUnit))
		if err != nil {
...
func RequestEpay(c *gin.Context) {
	...
	if operation_setting.GetQuotaDisplayType() == operation_setting.QuotaDisplayTypeTokens {
		normalizedAmount = decimal.NewFromInt(req.Amount).
...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		topUp, quotaToAdd, credited, err := model.CompleteEpayTopUp(verifyInfo.ServiceTradeNo, verifyInfo.Type, verifyInfo.Money)
		if err != nil {
