...
| `SESSION_COOKIE_TRUSTED_URL` | Secure 模式必填：允許呼叫 refresh/logout 的完全相符 HTTPS Origin，多個值以英文逗號分隔；**必須包含瀏覽器實際存取的每個面板網域，不能只填 relay API 網域**——漏掉面板網域會導致前端在登入約 15 分鐘後被登出（`POST /api/user/auth/refresh` 回傳 403 `AUTH_ORIGIN_FORBIDDEN`）；不是 relay CORS 白名單 | - |
